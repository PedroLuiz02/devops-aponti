...
- `data.aws_caller_identity.current` — retorna dados da conta AWS autenticada no momento (usado aqui para o `account_id`).
- `data.aws_region.current` — retorna a região configurada no provider.

O `outputs.tf` expõe essas informações (e dados do próprio bucket) para consulta rápida no terminal, sem precisar entrar no console da AWS:
